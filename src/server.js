...
app.post('/', (req, res) => {
    let resultado = '';
    let valorCotacao = req.params.cotacao;

    if (req.body.valorRef) {
        if (req.body.selectTemp == 1) {
            resultado = conversor.celsiusFahrenheit(req.body.valorRef, req.body.cotacao)
        } else {
            resultado = conversor.fahrenheitCelsius(req.body.valorRef, req.body.cotacao)
        }
    }
